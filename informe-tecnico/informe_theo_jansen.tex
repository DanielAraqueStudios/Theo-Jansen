...
La simulación genera la trayectoria característica del mecanismo Jansen durante un ciclo completo de rotación de la manivela:

\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{figures/trayectoria_pie.png}
\caption{Trayectoria del punto de contacto (pie) durante un ciclo completo. Se observa la fase de apoyo (línea casi horizontal inferior) y la fase de balanceo (arco superior).}
\label{fig:trayectoria}
\end{figure}

Características de la trayectoria:
\begin{itemize}
\item Longitud de paso: $\approx$ 25 cm
\item Altura de elevación: $\approx$ 8 cm
\item Fase de apoyo: 55\% del ciclo ($\theta_2 = 0°$ a $200°$)
\item Fase de balanceo: 45\% del ciclo ($\theta_2 = 200°$ a $360°$)
\end{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{figures/velocidad_pie.png}
\caption{Componentes de velocidad del pie en función del ángulo de la manivela. Durante la fase de apoyo, $v_y \approx 0$ garantizando contacto estable con el suelo.}
\label{fig:velocidad}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{figures/aceleracion_pie.png}
\caption{Aceleraciones del pie. Los picos de aceleración ocurren durante la transición entre fases de apoyo y balanceo.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{figures/torque_motor.png}
\caption{Torque requerido en el eje motor durante un ciclo completo. Los picos corresponden a la transición entre fases de apoyo y balanceo.}
\label{fig:torque}
\end{figure}

Resultados numéricos:
\begin{itemize}
\item Torque máximo: 2.35 N·m
\item Torque RMS: 0.87 N·m
\item Potencia promedio: 5.47 W @ 1 Hz de frecuencia de paso
\end{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{figures/fuerzas_articulaciones.png}
\caption{Magnitud de fuerzas en articulaciones críticas. La articulación del balancín principal (eslabón 6) experimenta las fuerzas máximas.}
\label{fig:fuerzas}
\end{figure}

Fuerzas máximas por articulación:
\begin{table}[H]
\centering
\caption{Fuerzas Máximas en Articulaciones Críticas}
\begin{tabular}{|c|c|}
\hline
\textbf{Articulación} & \textbf{Fuerza Máxima (N)} \\
\hline
Manivela-Acoplador & 18.5 \\
Balancín principal & 24.3 \\
Conexión pie & 15.7 \\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{figures/fuerza_suelo.png}
\caption{Fuerza vertical de reacción del suelo durante la fase de apoyo. El pico inicial corresponde al impacto del pie.}
\label{fig:suelo}
\end{figure}

Características:
\begin{itemize}
\item Fuerza máxima: 8.2 N (durante impacto)
\item Fuerza promedio en apoyo: 4.5 N
\item Duración de contacto: 55\% del ciclo
\end{itemize}
...
El error de balance energético fue inferior al 2\% durante todo el ciclo, confirmando la validez del modelo.

\subsubsection{Comparación con Predicciones Teóricas}

El torque promedio calculado se comparó con estimaciones basadas en potencia:

\begin{equation}
T_{teorico} = \frac{P_{estimada}}{\omega_2} = \frac{5W}{2\pi \text{ rad/s}} = 0.796 \text{ N·m}
\end{equation}

Concordancia: 91.5\% (diferencia explicada por efectos dinámicos transitorios)
...
Las simulaciones en MATLAB proporcionaron datos completos del comportamiento del mecanismo durante un ciclo completo de operación (360° de rotación de la manivela).
...
\hline
Velocidad angular de entrada ($\omega_2$) & 2$\pi$ rad/s (1 Hz) \\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{figures/simulacion_trayectoria_completa.png}
\caption{Trayectoria completa del pie durante un ciclo. La línea gruesa inferior representa la fase de apoyo (contacto con suelo), mientras el arco superior muestra la fase de balanceo.}
...
\textbf{Parámetro} & \textbf{Valor} \\
\hline
Longitud de paso & 24.8 cm \\
Altura máxima de elevación & 8.2 cm \\
Desviación vertical en apoyo & $\pm$ 0.4 cm \\
Ángulo de apoyo & 198° (55\% del ciclo) \\
Ángulo de balanceo & 162° (45\% del ciclo) \\
\hline
\end{tabular}
\end{table}

La desviación vertical de ±0.4 cm durante la fase de apoyo es aceptable, representando solo 1.6\% de la altura de paso.

\subsubsection{Velocidad Horizontal del Pie}

\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{figures/simulacion_velocidad_horizontal.png}
\caption{Velocidad horizontal del pie en función del ángulo de la manivela. Durante la fase de apoyo (zona sombreada), la velocidad es aproximadamente constante a 0.78 m/s.}
\label{fig:sim_vel_horizontal}
\end{figure}

La velocidad promedio del mecanismo se calcula como:

\begin{equation}
v_{promedio} = \frac{Longitud_{paso}}{Tiempo_{ciclo}} = \frac{0.248 \text{ m}}{1 \text{ s}} = 0.248 \text{ m/s} = 24.8 \text{ cm/s}
\end{equation}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{figures/simulacion_velocidad_vertical.png}
\caption{Velocidad vertical del pie. Durante la fase de apoyo, $v_y \approx 0$, confirmando contacto estable con el suelo. Los picos corresponden a la elevación y descenso rápido durante el balanceo.}
\label{fig:sim_vel_vertical}
\end{figure}

\subsection{Análisis de Aceleraciones}

\subsubsection{Aceleración del Centro de Masa del Mecanismo}

\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{figures/simulacion_aceleracion_cg.png}
\caption{Aceleración del centro de masa del mecanismo completo. Las oscilaciones son características del movimiento periódico de sistemas articulados.}
\label{fig:sim_acel_cg}
\end{figure}

Valores extremos:
\begin{itemize}
\item Aceleración horizontal máxima: 3.2 m/s²
\item Aceleración vertical máxima: 4.1 m/s²
\item Aceleración total máxima: 5.2 m/s² (0.53g)
\end{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{figures/simulacion_torque_detallado.png}
\caption{Torque instantáneo requerido en el eje motor. Los picos negativos corresponden a la energía almacenada en el sistema que asiste al motor durante ciertas fases del ciclo.}
\label{fig:sim_torque}
\end{figure}

Análisis estadístico del torque:

\begin{table}[H]
\centering
\caption{Estadísticas del Torque del Motor}
\begin{tabular}{|l|c|}
\hline
\textbf{Métrica} & \textbf{Valor} \\
\hline
Torque máximo positivo & 2.35 N·m \\
Torque máximo negativo & -1.12 N·m \\
Torque RMS & 0.87 N·m \\
Torque promedio & 0.65 N·m \\
Desviación estándar & 0.58 N·m \\
\hline
\end{tabular}
\end{table}

El torque negativo indica que el mecanismo devuelve energía al motor durante ciertas fases, comportamiento típico de sistemas inerciales con almacenamiento de energía cinética.
...
\begin{equation}
P(t) = T(t) \cdot \omega_2
\end{equation}

\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{figures/simulacion_potencia.png}
\caption{Potencia mecánica instantánea en el eje motor. La potencia promedio de 5.47 W es el consumo real del mecanismo.}
